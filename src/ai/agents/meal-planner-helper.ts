 ...
 * ```
 */
export async function callMealPlannerAgent(message: string, options: MealPlannerOptions) {
  const { familyId, threadId = randomUUID(), requestContext = new RequestContext<AugusteRequestContext>() } = options;
