 ...
 * Uses the init-orchestrator-agent for conversational setup.
 */

import * as readline from 'readline';
import { randomUUID } from 'crypto';
import { initOrchestratorAgent } from '../mastra/agents';
...
async function runInitFlow(): Promise<void> {
  ...
  try {
    ...
    const initialResponse = await initOrchestratorAgent.generate(
    ...
    const askQuestion = (): void => {
      rl.question(formatUserPrompt(), async (userInput) => {
        ...
        try {
          ...
          const response = await initOrchestratorAgent.generate(trimmedInput, { threadId });
