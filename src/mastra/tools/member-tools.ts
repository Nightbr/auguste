...
export const getMemberTool = createTool({
  ...
  inputSchema: z.object({
    id: z.string().uuid().describe('The member ID'),
  }),
...
export const getMemberByNameTool = createTool({
  ...
  inputSchema: z.object({
    familyId: z.string().uuid().describe('The family ID to search within'),
...
export const deleteMemberTool = createTool({
  ...
  inputSchema: z.object({ id: z.string().uuid().describe('The member ID to delete') }),
  outputSchema: z.object({ success: z.boolean(), deletedId: z.string().uuid() }),
  execute: async ({ id }) => {
    const db = getDatabase();
