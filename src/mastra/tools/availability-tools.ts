 ...
 */
function findMemberByName(db: ReturnType<typeof getDatabase>, familyId: string, memberName: string): MemberRow | undefined {
  return db.prepare(
    'SELECT id, name FROM Member WHERE familyId = ? AND LOWER(name) LIKE LOWER(?)'
  ).get(familyId, `%${memberName}%`) as MemberRow | undefined;
}
