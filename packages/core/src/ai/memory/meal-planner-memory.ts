...
const PlannerSettingsMemorySchema = PlannerSettingsSchema.omit({
  createdAt: true,
  updatedAt: true,
});
...
export const MealPlannerMemorySchema = z.object({
  ...
  plannerSettings: PlannerSettingsMemorySchema.optional().describe(
    'Configuration for meal planning',
  ),
  memberAvailability: z
    .array(MemberAvailabilitySchema)
    .default([])
    .describe('Availability schedule for all members'),

  // Current active plan state
  currentPlanning: MealPlanningMemorySchema.optional().describe(
    'The meal planning cycle currently being worked on',
  ),
  proposedEvents: z
    .array(MealEventMemorySchema)
    .default([])
    .describe('List of proposed meal events for the cycle'),
