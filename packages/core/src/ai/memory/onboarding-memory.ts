...
const OnboardingMemberSchema = z.object({
  id: z
    .uuid()
    .optional()
    .describe('Unique identifier for the member (assigned after database creation)'),
  ...
  hasAvailabilitySet: z
    .boolean()
    .default(false)
    .describe('Whether member availability has been collected'),
});
...
const OnboardingPlannerSettingsSchema = z.object({
  ...
  isConfigured: z
    .boolean()
    .default(false)
    .describe('Whether planner settings have been configured'),
});
...
export const OnboardingMemorySchema = z.object({
  family: OnboardingFamilySchema.partial()
    .optional()
    .describe('Family information (partially populated during onboarding)'),
  ...
  expectedMemberCount: z
    ...
    .min(1)
    .optional()
    .describe('Total number of members expected'),
  plannerSettings: OnboardingPlannerSettingsSchema.partial()
    .default({})
    .describe('Planner configuration settings'),
  currentPhase: z
    .enum([
